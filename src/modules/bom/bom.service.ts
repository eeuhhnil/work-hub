...
export class BOMService {
    ...
    try {
      ...
      const projectsWithStatus = await Promise.all(
        allProjects.map(async (project) => {
          const totalTasks = await this.db.task.countDocuments({ project: project._id })
      ...
      const completedProjects = projectsWithStatus.filter(p => p.isCompleted).length
      const overdueProjects = projectsWithStatus.filter(p => p.isOverdue).length
      const inProgressProjects = projectsWithStatus.filter(p => p.isInProgress).length
      ...
      const overduePercentage = totalProjects > 0 ? Math.round((overdueProjects / totalProjects) * 100) : 0
      ...
      const onTimeCompletionRate = totalProjects > 0 ? Math.round((completedProjects / totalProjects) * 100) : 0
    ...
    try {
      ...
      const spacePerformance = await Promise.all(
        spaces.map(async (space) => {
          ...
          console.log(`${space.name} - In-progress projects:`, inProgressProjects)
          ...
          const completedOnTimePercentage = totalProjects > 0
  ...
  async getPMPerformance(): Promise<PMPerformanceDto[]> {
    try {
      ...
      const spaceOwners = await this.db.spaceMember.find({
        role: 'owner' // SpaceRole.OWNER
      }).populate('space').populate('user')
      ...
      const pmPerformance = await Promise.all(
        spaceOwners.map(async (spaceMember) => {
          ...
          console.log(`Processing PM (Space Owner): ${pm.fullName} for space ${space.name}`)
          ...
          console.log(`${pm.fullName} - In-progress projects:`, inProgressProjects)
  ...
  async getRiskProjects(): Promise<RiskProjectDto[]> {
    try {
      ...
      const activeProjects = await this.db.project.find({
      ...
      const riskProjects = await Promise.all(
        activeProjects.map(async (project) => {
          ...
          const projectManager = await this.db.projectMember.findOne({
            project: project._id,
            role: ProjectRole.OWNER
          }).populate('user')

          const pmName = (projectManager?.user as any)?.fullName || 'Chưa phân công'
  ...
  async getKPIMetrics(): Promise<KPIMetricsDto> {
    try {
      ...
      const projectOnTimeCompletionRate = totalProjects > 0
      ...
      const taskOnTimeCompletionRate = totalTasks > 0
      ...
      if (completedProjectsWithDates.length > 0) {
        const totalDuration = completedProjectsWithDates.reduce((sum, project) => {
          ...
          const duration = Math.ceil((projectWithTimestamps.updatedAt.getTime() - projectWithTimestamps.createdAt.getTime()) / (1000 * 60 * 60 * 24))
          return sum + Math.max(1, duration) // At least 1 day
        }, 0)
        averageProjectDuration = Math.round(totalDuration / completedProjectsWithDates.length)
      }
  ...
  async getWeeklyProgress(): Promise<WeeklyProgressDto[]> {
    try {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const weekNumber = Math.ceil(((weekStart.getTime() - new Date(year, 0, 1).getTime()) / 86400000 + 1) / 7)
